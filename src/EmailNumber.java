...
public class EmailNumber {
            ...
            for(HashMap<String,String> key:map1.keySet())
            {
                ...
                for(String key1:map.keySet())
                {
                    ...
                    for (int i = 3; i <strNew.length ; i++) {
                        if(i==strNew.length-1||i==strNew.length-2||i==strNew.length-3||i==strNew.length-4||i==strNew.length-5||i==strNew.length-6||i==strNew.length-7||i==strNew.length-9){
                            continue;
                        }
                        if(i== strNew.length-8){
                            strNew[i] = key1;
                        }
                        str += strNew[i]+",";
                    }
    ...
    public static void write(String path, String str1, String str2) {
        try {
            File csv = new File("email1/" + path); // CSV数据文件
           // File csv = new File("G:\\开题\\data_new\\email1\\" + path); // CSV数据文件
    ...
    public static void main(String[] args) {
        EmailNumber fw = new EmailNumber();
        //read("G:\\开题\\data_new\\device\\device_AAC06102009_12.csv");
        fw.walk("email" );
        //fw.walk("G:\\开题\\data_new\\email");
    }
